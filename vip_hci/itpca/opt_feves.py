...
def feves_opt(cube, angle_list, cube_ref=None, fwhm=4, strategy='ADI', 
    ...
    *** INSERT LINK ***    
    ...
    #     delta_rot_tests = ([delta_rot],[delta_rot])
    ...
    #         delta_rot_tests = ([delta_rot[0]],[delta_rot[1]])
    ...
    if nproc>1:
        res = pool_map(nproc, _opt_buff_feve, iterable(range(buff)), cube_tmp, 
                       ...
                       imlib, interpolation, edge_blend, path, debug, overwrite, 
                       smooth)
        ...
        for bb in range(buff):
            res = _opt_buff_feve(bb, cube_tmp, angle_list, ref_cube_tmp, algo, 
                                 ...
                                 interpolation, edge_blend, path, debug, 
                                 overwrite, smooth)
...
def _opt_buff_feve(bb, cube, angle_list, ref_cube=None, algo=pca_annular, 
                   ...
                   interpolation='lanczos4', edge_blend=None, path='', 
                   debug=False, overwrite=False, smooth=False):
    # select fm function
    if fm == 'med_stim':
        ffm = np.median
    elif fm == 'mean_stim':
                ...
                for pp, ncomp in enumerate(ncomps):
                    ...
                    if not debug or not isfile(fn+"_norm_stim_cube.fits") or overwrite:
                        res = feves(cube, angle_list, cube_ref=ref_cube, 
                                    ...
                                    init_svd=init_svd, nproc=nproc, imlib=imlib, 
                                    interpolation=interpolation, 
                                    edge_blend=edge_blend, full_output=True,
