...
from astropy.convolution import convolve as ast_convolve
import numpy as np
from scipy.interpolate import InterpolatedUnivariateSpline
from ..conf.utils_conf import pool_map, iterable
from .pca_fullfr import _interp2d_rad, throughput_ext
...
def pca_annular_it(cube, angle_list, cube_ref=None, ncomp=1, n_it=10, thr=1.,
                   ...
                   imlib='opencv', interpolation='lanczos4', edge_blend=None,
                   collapse='median', full_output=False, verbose=True,
                   weights=None, add_res=False, interp_order=2, rtol=1e-2,
                   atol=1):
    """
    ...
    def _find_significant_signals(residuals_cube, residuals_cube_, angle_list,
        ...
        if thr_per_ann:
            ...
            for aa in range(n_ann):
                ...
                norm_stim[np.where(asec != 0)] = stim[np.where(
                    asec != 0)]/max_inv
    ...
    # convert to even-size for FFT-based rotation
    if imlib == 'vip-fft' and cube.shape[-1] % 2:
    ...
    res = pca_annular(cube, angle_list, cube_ref=ref_cube, radius_int=radius_int,
                      ...
                      weights=weights, edge_blend=edge_blend)
    ...
    for it in range(1, n_it+1):
        ...
        sig_cube = cube_derotate(sig_cube, -angle_list, imlib=imlib,
                                 edge_blend=edge_blend, nproc=nproc)
...
def feves(cube, angle_list, cube_ref=None, ncomp=1, algo=pca_annular, n_it=2,
          ...
          psfn=None, n_br=6, radius_int=0, delta_rot=1, svd_mode='lapack',
          init_svd='nndsvda', nproc=1, min_frames_lib=2, max_frames_lib=200,
          tol=1e-1, scaling=None, imlib='vip-fft', imlib2='opencv',
          interpolation='lanczos4', edge_blend=None, collapse='median',
          smooth=False, full_output=False, verbose=True, weights=None,
    ...
    imlib : str, optional
        See the documentation of the ``vip_hci.preproc.frame_rotate`` function.
        Used for non-azimuthally segmented cases (e.g. 'vip-fft' is usually a
        good option).
    imlib2 : str, optional
        See the documentation of the ``vip_hci.preproc.frame_rotate`` function.
        Used for azimuthally segmented cases, i.e. n_segments>1 ('vip-fft' is 
    ...
    if strategy == 'ADI':
        ...
        if cube_ref is None:
            raise ValueError(
                "cube_ref should be provided for RDI, RADI or ARDI")
        ref_cube = cube_ref.copy()
        if strategy == 'RDI':
            # forces doing RDI instead of ARDI
            delta_rot = cube.shape[1]*3/fwhm
    else:
    ...
    if nproc > 1:
        res = pool_map(nproc, _do_one_buff, iterable(range(buffer)), cube,
                       ...
                       max_frames_lib, tol, scaling, imlib, imlib2, interpolation,
        ...
        for bb in range(buffer):
            res = _do_one_buff(bb, cube, angle_list, ref_cube, algo, n_it, thr,
                               ...
                               max_frames_lib, tol, scaling, imlib, imlib2,
                               interpolation, collapse, atol, rtol, nproc_tmp,
                               True, verbose, weights, smooth, edge_blend)
...
def _do_one_buff(bb, cube, angle_list, ref_cube, algo, n_it, thr, thr_per_ann,
                 ...
                 scaling, imlib, imlib2, interpolation, collapse, atol, rtol,
                 nproc, full_output, verbose, weights, smooth, edge_blend):

    def _blurring_2d(array, mask_center_sz, fwhm_sz=2):
        if mask_center_sz:
            frame_mask = mask_circle(array, radius=mask_center_sz+2*fwhm_sz,
    ...
    for it_f in range(n_it_frac):
        for pp, npc in enumerate(ncomp[it_f//n_it]):
            ...
            # frame[nonan_loc] = frame_filt[nonan_loc]
            # write_fits("TMP_sig_image_blur.fits",frame)
            if smooth or (n_segments[it_f//n_it] > 1 and imlib_seg == 'vip-fft'):
                frame = _blurring_2d(frame, radius_int, fwhm_sz=2)
                # write_fits("TMP_sig_image_blur.fits",frame)
            # create and rotate sig cube
            sig_cube = np.repeat(frame[np.newaxis, :, :], nframes, axis=0)
            sig_cube = cube_derotate(sig_cube, -angle_list, imlib=imlib_seg,
                                     edge_blend=edge_blend, nproc=nproc)
            # write_fits("TMP_sig_cube.fits",sig_cube)
            # create and rotate binary mask
            mask_sig = np.zeros_like(sig_image)
           ...
           # write_fits("TMP_derot_msig_image.fits",sig_image)
            sig_mcube = np.repeat(mask_sig[np.newaxis, :, :], nframes, axis=0)
            # write_fits("TMP_rot_msig_cube.fits",sig_mcube)
            sig_mcube = cube_derotate(sig_mcube, -angle_list, imlib=imlib_interp,
                                      interpolation='bilinear', nproc=nproc)
            sig_cube[np.where(sig_mcube < 0.5)] = 0
            #sig_cube = mask_circle(sig_cube, radius_int, 0)
           # write_fits("TMP_derot_msig_cube.fits", sig_mcube)
           ...
           # pdb.set_trace()
            ...
            for tt in range(n_segments[it_f//n_it]):
                ...
                if algo == pca_annular:
                    res_it = pca_annular(cube, angle_list, cube_ref=ref_cube,
                                         ...
                                         min_frames_lib=min_frames_lib,
                                         max_frames_lib=max_frames_lib, tol=tol,
                                         scaling=scaling, imlib=imlib_seg,
                                         interpolation=interpolation,
            ...
            if smooth or (n_segments[it_f//n_it] > 1 and imlib_seg == 'vip-fft'):
                residuals_cube = _blurring_3d(residuals_cube, radius_int,
                                              fwhm_sz=2)
                residuals_cube_ = cube_derotate(residuals_cube, angle_list,
                                                imlib=imlib_seg, nproc=nproc,
                                                edge_blend=edge_blend)
            ...
            if add_res and n_neigh == 0:
                ...
                sig_image[np.where(sig_mask_p)
                          ] += frame_nd[np.where(sig_mask_p)]  # c
                sig_image[np.where(sig_image < 0)] = 0
            # tmp=frame.copy()

            frame[np.where(frame > 0)] = frame[np.where(
                frame > 0)]/thru_2d[np.where(frame > 0)]
