...
from ..nmf import nmf
from ..preproc import cube_derotate, cube_collapse#, frame_crop, cube_shift, frame_pad
from ..pca import pca
...
from ..var import (prepare_matrix, mask_circle, cart_to_pol, frame_center,
                   frame_filter_lowpass)#,  get_annulus_segments)
...
def pca_it(cube, angle_list, cube_ref=None, algo=pca, mode=None, ncomp=1, 
           ...
           smooth=False, add_res=False, rtol=1e-2, atol=1, edge_blend=None,
           **kwargs_nmf):
    """
    ...
    if edge_blend is None:
        # avoid mask-edge artefacts induced by rotation, by filling with 
        # results obtained with no mask
        mask_center_px = None
    ...
    # NO NEED TO CONVERT TO EVEN SIZE - easy conversion before FFT!
    # convert input cube to even size for FFT rotations
    # if imlib2 == 'vip-fft' and cube.shape[-1]%2:
    #     # cube_pad = []
    #     # for i in range(cube.shape[0]):
    #     #     cube_pad.append(frame_pad(cube[i], 4, fillwith=0))
    #     #     if i == 0:
    #     #         ori_idx = frame_pad(cube[i], 4, fillwith=0, full_output=True)[1]
    #     # cube_pad = np.array(cube_pad, dtype=object)
    #     cube = cube_shift(cube, 0.5, 0.5)#, imlib='ndimage-fourier')
    #     cube = cube[:,1:,1:]

    # if ref_cube is not None:
    #     if imlib2 == 'vip-fft' and ref_cube.shape[-1]%2:    
    #         ref_cube = cube_shift(ref_cube, 0.5, 0.5)#, imlib='ndimage-fourier')        
    #         ref_cube = ref_cube[:,1:,1:]    
    ...
    for it in range(1,n_it):
        ...
        sig_mcube = cube_derotate(sig_mcube, -angle_list, imlib=imlib2, 
