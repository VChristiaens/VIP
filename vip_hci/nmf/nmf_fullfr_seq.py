...
def nmf_seq(cube, angle_list, cube_err=None, cube_ref=None, cube_ref_err=None, 
    ...
    handle_neg: str, opt {'mask','subtr_min','null'}
    ...
    if handle_neg == 'mask':
        ...
        if handle_neg=='null':
            if cube_sig is not None:
                matrix[np.where(matrix-matrix_sig<0)]=0
                matrix_sig[np.where(matrix-matrix_sig<0)]=0
            else:
                matrix[np.where(matrix<0)]=0
            
        elif handle_neg=='subtr_min':
            if cube_sig is not None:
                matrix -= np.amin(matrix-matrix_sig)
            else:
    ...
    if cube_sig is not None:
        ...
        residuals_cube = array.copy()
        if cube_ref is not None:
            med_model = np.nanmedian(cube_ref, axis=0)
        else:
            med_model = np.nanmedian(array, axis=0)
        for n in range(residuals_cube.shape[0]):
